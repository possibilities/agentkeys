...
STATE_DIR="${AGENTKEYS_INSTALL_STATE_DIR:-$HOME/.local/state/agentkeys}"
TARGET="$BIN_DIR/agentkeys"
RECEIPT="$STATE_DIR/deployed-sha"
UPSTREAM_ORIGIN="https://github.com/possibilities/agentkeys.git"
...
EXPECTED_ORIGIN="${AGENTKEYS_INSTALL_EXPECTED_ORIGIN:-$UPSTREAM_ORIGIN}"
TMP_PATH=""
...
With no option, installs agentkeys. Installation runs Bun's frozen dependency
install, links ~/.local/bin/agentkeys to this checkout, and writes the deployed
Git SHA to ~/.local/state/agentkeys/deployed-sha.

Set AGENTKEYS_INSTALL_BIN_DIR and AGENTKEYS_INSTALL_STATE_DIR to use other
...
normalized_origin() {
  ...
  origin="${origin%/}"
  case "$origin" in
    https://github.com/possibilities/agentkeys|https://github.com/possibilities/agentkeys.git)
      printf '%s\n' "$UPSTREAM_ORIGIN"
      ;;
    git@github.com:possibilities/agentkeys|git@github.com:possibilities/agentkeys.git|ssh://git@github.com/possibilities/agentkeys|ssh://git@github.com/possibilities/agentkeys.git)
      printf '%s\n' "$UPSTREAM_ORIGIN"
      ;;
    *)
      printf '%s\n' "$origin"
...
validate_managed_checkout() {
  ...
  validate_safe_file "$source" "agentkeys source command"
  [[ -x "$source" ]] || die "Refusing non-executable agentkeys source command: $source"
  sha="$(checkout_head "$root")" || die "Refusing agentkeys source outside an exact Git checkout: $root"
  origin="$(git -C "$root" remote get-url origin 2>/dev/null)" || die "Refusing agentkeys source without an origin: $root"
  [[ "$(normalized_origin "$origin")" == "$EXPECTED_ORIGIN" ]] || die "Refusing agentkeys source with foreign origin: $root"
  MANAGED_ROOT="$root"
  MANAGED_SHA="$sha"
}
...
install_agentkeys() {
  ...
  TMP_PATH="$BIN_DIR/.agentkeys-link.$$.$RANDOM"
...
uninstall_agentkeys() {
  ...
  if (( removed )); then
    echo "Removed agentkeys installation"
  else
    echo "Agentkeys is not installed"
  fi
}
